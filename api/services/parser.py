...
def extract_boleto_info(pdf_path):
    ...
    result = {
        "recipient": find_match(text_content, r'(?:Beneficiário|Cedente)[\s.:\n]*([\s\S]*?)(?=\n\s*\b(?:Agência\s*/\s*Código|Nosso\sNúmero|Sacado|Pagador|Vencimento|Local\sde\sPagamento)\b)'),
        "drawee": find_match(text_content, r'(?:Pagador|Sacado)[\s.:\n]*?([\s\S]*?)(?=\n\s*\b(?:Instruções|Descrição|Autenticação Mecânica|FICHA DE COMPENSAÇÃO|Nº\sdo\sDocumento|Data\sdo\sDocumento)\b)'),
