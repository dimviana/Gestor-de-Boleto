...
def extract_boleto_info(pdf_path):
    ...
    # IMPROVED: Regex looks for a date within a 30-char window after the label for better precision
    due_date_regex = r'Vencimento(?:[\s\S]{0,30}?)(\d{2}[/\s.Il]?\d{2}[/\s.Il]?\d{4})'
    doc_date_regex = r'Data (?:do )?Documento(?:[\s\S]{0,30}?)(\d{2}[/\s.Il]?\d{2}[/\s.Il]?\d{4})'
