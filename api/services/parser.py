...
def extract_boleto_info(pdf_path):
    ...
    amount_str = find_match(text_content, r'Valor Cobrado[^\d\n,]*((?:R\$\s*)?[\d.,]+)', find_last=True)
    doc_amount_str = find_match(text_content, r'Valor (?:do )?Documento[^\d\n,]*((?:R\$\s*)?[\d.,]+)', find_last=True)
    ...
    result = {
        ...
        "documentDate": parse_date(find_match(text_content, r'Data (?:do )?Documento[^\d\n]*(\d{2}[/\s.Il]?\d{2}[/\s.Il]?\d{4})', find_last=True)),
        "dueDate": parse_date(find_match(text_content, r'Vencimento[^\d\n]*(\d{2}[/\s.Il]?\d{2}[/\s.Il]?\d{4})', find_last=True)),
        "documentAmount": doc_amount,
        "amount": amount,
        "discount": parse_currency(find_match(text_content, r'(?:Desconto\s*/\s*Abatimento)[^\d\n,]*((?:R\$\s*)?[\d.,]+)', find_last=True)),
        "interestAndFines": parse_currency(find_match(text_content, r'(?:Juros\s*/\s*Multa|Outros Acréscimos)[^\d\n,]*((?:R\$\s*)?[\d.,]+)', find_last=True)),
