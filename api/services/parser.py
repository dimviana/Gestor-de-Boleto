...
def extract_boleto_info(pdf_path):
    ...
    # IMPROVED: Recipient regex with more delimiters to avoid over-capturing.
    recipient_regex = r'(?:Beneficiário|Cedente)[\s.:\n]*([\s\S]*?)(?=\n\s*\b(?:Agência\s*/\s*Código|Nosso\sNúmero|Sacado|Pagador|Vencimento|Local\sde\sPagamento|Data (?:do )?Documento)\b)'
    
    # IMPROVED: Drawee regex stops before 'Sacador / Avalista' to capture only the name.
