...
def find_pix_code(text):
    ...
    # PIX codes start with '000201' and often end with '6304XXXX' (CRC).
    # This regex captures the whole string, removing newlines which OCR might insert.
    match = re.search(r'(000201[a-zA-Z0-9*@$.%+/=_-]{100,})', text)
    if match:
...
def extract_boleto_info(pdf_path):
    ...
    PATTERNS = {
        # Recipient: Look for the label and capture the text on the same or next line until another known label appears.
        "recipient": r'(?i)(?:Beneficiário|Cedente)[\s:.]*\n?([\s\S]*?)(?=\n\s*(?:CNPJ:|Agência|Nosso Número|Sacado|Pagador))',
        # Drawee: Similar logic to recipient.
        "drawee": r'(?i)(?:Pagador|Sacado)[\s:.]*\n?([\s\S]*?)(?=\n\s*(?:CNPJ:|CPF:|Endereço|Sacador|Avalista|Instruções|Autenticação Mecânica))',
        # Due Date: Find the label and look for a date pattern on the same line or the next.
        "dueDate": r'(?i)Vencimento\n?.*?\b(\d{2}[/.]\d{2}[/.]\d{4})\b',
        # Document Date: Same logic as due date.
        "documentDate": r'(?i)Data (?:do )?Documento\n?.*?\b(\d{2}[/.]\d{2}[/.]\d{4})\b',
        # Amount: Prioritize "Valor Cobrado", then "Valor do Documento". The value is often at the end of the line.
        "amount_cobrado": r'(?i)\(=\) Valor Cobrado\n?((?:R\$\s*)?[\d.,]+)',
        "amount_documento": r'(?i)Valor (?:do )?Documento\n?((?:R\$\s*)?[\d.,]+)',
        # Other values:
        "discount": r'(?i)\(-\) (?:Desconto|Abatimento)\n?((?:R\$\s*)?[\d.,]+)',
        "interestAndFines": r'(?i)\(\+\) (?:Juros|Multa|Mora|Acréscimos)\n?((?:R\$\s*)?[\d.,]+)',
        # Document Number: Prioritize "Nº Documento", then "Nosso Número".
        "guideNumber_doc": r'(?i)N[ºo\.]?\s?(?:do\s)?Documento[\s.:\n]*?([^\n\s]+)',
        "guideNumber_nosso": r'(?i)Nosso\sN[úu]mero[\s.:\n]*?([^\n\s]+)',
    }

    def find_match(pattern_key, text):
        match = re.search(PATTERNS[pattern_key], text)
        return clean_text(match.group(1)) if match else None

    # --- Extraction Execution ---
    
    # Extract raw string values using the patterns
    raw = {
        "recipient": find_match("recipient", text_content),
        "drawee": find_match("drawee", text_content),
        "dueDate": find_match("dueDate", text_content),
        "documentDate": find_match("documentDate", text_content),
        "amount_cobrado": find_match("amount_cobrado", text_content),
        "amount_documento": find_match("amount_documento", text_content),
        "discount": find_match("discount", text_content),
        "interestAndFines": find_match("interestAndFines", text_content),
        "guideNumber": find_match("guideNumber_doc", text_content) or find_match("guideNumber_nosso", text_content),
